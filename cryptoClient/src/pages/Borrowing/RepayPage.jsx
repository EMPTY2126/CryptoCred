...
function RepayPage(){
  ...
  const sendEth = async()=>{
    ...
    try {
      ...
      const newState = {
        ...loan,
        heading: "Repayment Successful!",
        secondheading: "Your repayment transaction is successfull."
      }
      navigate("/requestConfirmation", {state:newState});
    } catch (error) {
      ...
      >
        ...
        <div className="absolute top-[-1px] left-[-1px] rounded-lg shadow-sm mx-auto h-full w-full p-6 bg-black opacity-70 flex justify-center items-center" style={{ zIndex: 1 }}>
          ...
          >
            ...
            <p className="text-gray-300">
              <strong>Total Repayment:</strong> {pay} ETH
            </p>
          </div>
          ...
          <div className="mt-6">
            <button
              className="mt-4 bg-green-500 hover:bg-green-600 p-[3%] text-white font-bold  rounded w-full"
              onClick={sendEth}
            >
              Pay {pay}
            </button>
